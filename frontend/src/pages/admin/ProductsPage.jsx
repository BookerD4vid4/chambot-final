...
const ProductsPage = () => {
    ...
    const handleToggleActive = async (p) => {
        try {
            ...
            await updateProduct(p.product_id, { name: p.product_name || p.name, is_active: newStatus });
            toast.success(newStatus ? 'เปิดการใช้งานสินค้าสำเร็จ' : 'ปิดการใช้งานสินค้าสำเร็จ');
    ...
    const ocrHandleFile = (file) => {
        ...
        if (file.size > 20 * 1024 * 1024) {
            toast.error('ไฟล์มีขนาดใหญ่เกินไป (สูงสุด 20 MB) กรุณาลดขนาดรูปภาพก่อนอัปโหลด', { duration: 5000 });
            return;
        }
    ...
    const ocrScanImage = async () => {
        ...
        try {
            ...
            if (status === 413) {
                toast.error(msg || 'ไฟล์มีขนาดใหญ่เกินไป (สูงสุด 20 MB) กรุณาลดขนาดรูปภาพก่อนอัปโหลด', { duration: 6000 });
            } else {
                toast.error(msg || 'วิเคราะห์รูปภาพไม่สำเร็จ กรุณาลองใหม่อีกครั้ง');
            }
                    ...
                    <div className="modal-box prod-detail-modal" onClick={e => e.stopPropagation()}>
                                ...
                                <div className="prod-detail-grid">
                                    ...
                                    <div className="prod-detail-img-col">
                                        <div className="prod-detail-img">
                                            {(() => {
                                                const mainVariant = selectedProduct.variants?.find(v => v.is_main);
                                                const displayUrl = mainVariant?.image_url || selectedProduct.image_url;
                                                return <ProductImage src={displayUrl} alt={selectedProduct.product_name} />;
                                            })()}
                                        </div>
                        ...
                        <div className="modal-footer" style={{ padding: '16px 28px 24px', gap: 12 }}>
                            ...
                            <button className="btn btn-primary"
                                style={{
                                    minHeight: 52, fontSize: 16, gap: 8, flex: 1,
                                    ...(selectedProduct.is_active ? {} : { background: '#10b981', borderColor: '#10b981' })
                                }}
                                onClick={() => {
                                ...
                                {selectedProduct.is_active ? (
                                    ...
                                    <><CheckCircle size={18} /> เปิดการใช้งานสินค้า</>
                                )}
                            </button>
                        </div>
                    </div>
                                    ...
                                    >
                                        {ocrPreview
                                            ...
                                            : (<>
                                                ...
                                                <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>รองรับ JPG, PNG, WEBP — สูงสุด 20MB</span>
                                            </>)
                                        }
                                                ...
                                                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: 13 }}>
                                                    {item.is_new ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <div><span style={{ color: 'var(--text-muted)', fontSize: 11 }}>ราคาปัจจุบัน</span><div style={{ color: '#AA5DC6', fontWeight: 600 }}>{item.existing_variant_price != null ? `฿${Number(item.existing_variant_price).toLocaleString()}` : '—'}</div></div>
                                                            <div><span style={{ color: 'var(--text-muted)', fontSize: 11 }}>สต็อกปัจจุบัน</span><div style={{ color: 'var(--text-primary)' }}>{item.existing_variant_stock ?? '—'} {item.existing_variant_unit || ''}</div></div>
                                                    ...
                                                    {!item.is_new ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <div className="form-row">
                                                                <div className="input-group">
                                                                    <label className="input-label">หมวดหมู่</label>
                                                                    <select className="input-field" value={item.category_id || ''} onChange={e => ocrUpdateItem(idx, 'category_id', e.target.value)}>
                                                                        <option value="">-- ไม่ระบุ --</option>
                                                                        {categories.map(c => <option key={c.category_id} value={c.category_id}>{c.name}</option>)}
                                                                    </select>
                                                                </div>
                                                                <div className="input-group"><label className="input-label">แบรนด์</label><input className="input-field" value={item.brand || ''} onChange={e => ocrUpdateItem(idx, 'brand', e.target.value)} placeholder="เช่น Olay, Unilever" /></div>
                                                            </div>
                                                            <div className="form-row">
                                                                <div className="input-group">
